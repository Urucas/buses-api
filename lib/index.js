...
class Parser{
  company(dom){
    ...
    try {
      ...
      company.name = dom.children(".detalle_empresa")["0"].children[0].data.trim()
    }catch(e){}
    return company 
  }
  leaves(dom){
      ...
      (err, response, body) => {
        ...
        for(let i=0; i<rows.length;i++){
          ...
          let travel = {
            ...
            company: parser.company(row(".empresa")),
            leaves: parser.leaves(row(".sale")),
            arrives: parser.arrives(row(".llega")),
            days: parser.days(row(".dias")),
            service: parser.service(row("td"))
          }
          travels.push(travel)
        }
        res(travels)
